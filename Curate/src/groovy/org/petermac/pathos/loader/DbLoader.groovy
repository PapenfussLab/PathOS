...
class DbLoader
{
    ...
    void mergeETL( String dbase )
    {
        ...
        //        Deprecated: KDD 25 Sep
        //        Replaced by VcfLoader Class
        //
        //        int svcnt = LoadSeqVariant.addSeqVariant( sql, dbase )
        ...
        /*if ( svcnt ) AES commenting this out but KDD since addSeqVariant is deperecated we need a new way to do this.
        {
     ...
     * @return  Count of rows added
     */
    int addPatSamples(boolean merge )
    {
    ...
    {
        log.info( 'Adding Patient Assays')
        ...
        for ( row in sts )
        {
            ...
            //
            LabAssay labAssay = LabAssay.findByTestName((row.test_desc).trim())
    ...
    int addSeqSamples( boolean merge )
    {
        ...
        for ( row in rows )
        {
            ...
            //	Lookup sample
            //
            String sn = SampleName.impliedPatientSampleName(row.sample)
            def patSample = PatSample.findBySample(sn)
            if ( ! patSample )
            {
                log.debug( "Row: ${rowcnt} Couldn't find patient sample [${row.sample}]")
            }
            ...
            def ss = new SeqSample(	seqrun:		seqrun,
                                    ...
                                    clinContext: ClinContext.generic()
                                    )
