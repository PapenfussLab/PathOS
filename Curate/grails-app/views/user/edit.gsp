...
<s2ui:tabs elementId='tabs' height='375' data="${tabData}">
	...
	<s2ui:tab name='userinfo' height='275'>
       ...
       <tbody>
			...
			<g:if test="${! ('ldapAuthProvider' in grailsApplication.config.grails.plugin.springsecurity.providerNames)}">
			<s2ui:passwordFieldRow name='password' labelCode='user.password.label' bean="${user}"
                                labelCodeDefault='Password' value="${user?.password}"/>
			</g:if>

            <s2ui:textFieldRow name='displayName' labelCode='user.displayName.label' bean="${user}"
                               labelCodeDefault='Display Name' value="${user?.displayName}"/>
